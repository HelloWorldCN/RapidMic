...
### 参数:
	...
	-b <var index>, --pairsBetween=<var index>     前i数据和剩余数据之间两两比对，因此取值范围为(0, 数据个数)
	-m <var index>, --master=<var index>     指定一个数据和其它数据之间进行两两比对，取值范围为 [0, 数据个数);
...
在 Unix ,linux 和 macos 操作系统下采用下面的命令编译
