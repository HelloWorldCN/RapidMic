...
void mine_problem_init(mine_problem *prob, mine_parameter *param)
{
	...
	for (gy=2; gy<Gy_max+1; gy++)
	{
		...
		if ((gx * gy) <= B)
		{
			prob->score->p[gy-2] = gx-1;//最小划分为2格，如果gx为1则不用划分了，所以也就不用存储数据，如果为2则只有一个数据
			prob->Gy[gy-2] = gy;
		}
	}
	...
	prob->x_y = (double *) malloc (prob->n * sizeof(double));//对(xi,yi)的成对数据按照y的值进行排序后的x序列
	prob->y_x = (double *) malloc (prob->n * sizeof(double));//对(xi,yi)的成对数据按照x的值进行排序后的y序列
	for (j=0; j<prob->n; j++)
	{
...
void mine_problem_free(mine_problem *prob)
{
	free(prob->x_x);
	free(prob->y_y);
	free(prob->idx_x) ;
...
// Parameter: double * It
//************************************
void FixOnePartition(mine_problem *prob,double c,int firstpartition,int indexp,double *It,int *Qm,int *Qm_s)
{
	int k;
	int j, q, p;
	...
	if (firstpartition==0)
	{
		...
		for (j=0; j<prob->n; j++)
			Qm_s[j] = Qm[prob->idx_x[j]];//获得x在原始数据对(x,y)约束条件下x排序后数据划分表,也可以理解成和x对位的y所在划分格子
...
void *PartitionThread(void *pparam)
{
	...
	subI = (double **) malloc ((pthdparainf->indexEnd-pthdparainf->indexBegin) * sizeof(double *));
	for (i=pthdparainf->indexBegin;i<pthdparainf->indexEnd;i++)
	{
		subI[i-pthdparainf->indexBegin]=(double *) malloc ((pthdparainf->prob->score->p[i]) * sizeof(double));
		FixOnePartition(pthdparainf->prob,pthdparainf->c,pthdparainf->firstPartition,i,subI[i-pthdparainf->indexBegin],Qm, Qm_s);
	}
	...
	for (i=pthdparainf->indexBegin;i<pthdparainf->indexEnd;i++)
	{
		if (pthdparainf->firstPartition==0)
		{
			...
			for (j=0; j<pthdparainf->prob->score->p[i]; j++)
				pthdparainf->prob->score->I[i][j] = MAX(subI[i-pthdparainf->indexBegin][j],
                                                        pthdparainf->prob->score->I[i][j]);
		}else if (pthdparainf->firstPartition==1)
		{
			//compare i column
			for (j=0; j<pthdparainf->prob->score->p[i]; j++)
				pthdparainf->prob->score->I[j][i] = MAX(subI[i-pthdparainf->indexBegin][j],
...
void *batchComputeScoreThread(void *pparam)
{
	...
	Qm_s = (int *) malloc (prob.n * sizeof(int));//按照数据排序后顺序保存的数据划分表
	...
	for (i=0;i<pthdparainf->parisLength;i++)
	{
		...
		for (j=0; j<prob.n; j++)
		{
            if (i==0||(i!=0&&pthdparainf->paris[i].var1!=pthdparainf->paris[i-1].var1)) {//和上一轮不相同则需要重新赋值并排序
                prob.x_x[j] = prob.x[j];
                prob.idx_x[j] = j;
            }
			
            if (i==0||(i!=0&&pthdparainf->paris[i].var2!=pthdparainf->paris[i-1].var2)) {//和上一轮不相同则需要重新赋值并排序
                prob.y_y[j] = prob.y[j];
                prob.idx_y[j] = j;
            }
        ...
        /* sort x by y-value and sort y by x-value */
		//对原始序列值进行排序，同时记录排序后的原数据所处位置序列
        if (i==0||(i!=0&&pthdparainf->paris[i].var1!=pthdparainf->paris[i-1].var1))
            sort(prob.x_x, prob.idx_x, prob.n);
        if (i==0||(i!=0&&pthdparainf->paris[i].var2!=pthdparainf->paris[i-1].var2))
            sort(prob.y_y, prob.idx_y, prob.n);
		...
		switch (pthdparainf->styleType) {
            ...
            case TwoSets:
                ...
                break;
            case MasterVariable:
                get_result_score(&score,&pthdparainf->outArray[pthdparainf->paris[i].var1]);
                break;
            default:
                break;
        }
...
mine_score *mine_compute_score_multithread(mine_problem *prob, mine_parameter *param)
{
	...
	//可能需要修改一下
	pagecount=(2*prob->score->m)/NUM_THREADS;
	for(t=0; t<NUM_THREADS; t++){
		...
		if (t<(NUM_THREADS/2-1))
		{
			...
			pthreadinfos[t]->indexBegin=(t)*pagecount;
			pthreadinfos[t]->indexEnd=(t+1)*pagecount;
		...
		if (t>(NUM_THREADS/2-1)&&t<(NUM_THREADS-1))
		{
			...
			pthreadinfos[t]->indexBegin=(t-NUM_THREADS/2)*pagecount;
			pthreadinfos[t]->indexEnd=(t+1-NUM_THREADS/2)*pagecount;
			pthreadinfos[t]->prob=prob;
		}
		if (t==(NUM_THREADS-1))
		{
    ...
    printf( "%f seconds\n", duration );
	pthread_mutex_destroy(&thread_complete_lock);
	pthread_mutex_destroy(&res_lock);
	pthread_cond_destroy(&all_thread_completed);
	pthread_exit(NULL);
	system("pause");
	return prob->score;
...
int createBatchComputeThread(mine_parameter *param,double **inDataSet,int m,int n,int masterOrBetweenVariableid,mine_result_score *outArray,int outLen,ANALYSISSTYLES styleType)
{
	...
	for (gy=2; gy<Gy_max+1; gy++)
	{
		...
		if ((gx * gy) <= B)
		{
			p[gy-2] = gx-1;//最小划分为2格，如果gx为1则不用划分了，所以也就不用存储数据，如果为2则只有一个数据
			Gy[gy-2] = gy;
		}
	}
	if (m>10)
    ...
    if (styleType==MasterVariable) outLen--;
