...
void show_help() {
  ...
  std::printf(
      ...
      "-l <label style>, --inputlabel=<label style>     if input csv file's "
      "first line is comlumn name and the first comlumn is row name, then "
      "<label style>=0 ,else if just only comlumn name <label style>=1, else "
      "if just only row name <label style>=2 ;\n"
      "-a <alpha value>, --alpha=<alpha>     the exponent in B(n) = n^alpha "
      "(default: 0.6.) alpha must be in (0, 1.0]\n"
      "-c <clumps value>, --clumps=<c>     determines how many more clumps "
      "there will be than columns in every partition. Default value is 15,c "
      "must be > 0;\n"
      "-o <file>, --output=<file>     output filename (if not be set, will "
      "print result in screen);\n"
      "-L <label style>, --outputlabel=<label style>     output csv file adopt "
      "number index of row as vaiable label ,then <label style>=0，else adopt "
      "input file's row name, then <label style>=1;\n"
      "-A <allPairs>, --allPairs     will cause MINE to compare all pairs of "
      "variables against each other; \n"
      "-b <var index>, --pairsBetween=<var index>     will compare each of the "
      "first i variables to each of the rest of the variables. Variables are "
      "indexed from 0;input variable <var index> must be in (0, number of "
      "variables in file)\n"
      "-m <var index>, --master=<var index>     variable <var index> vs. all "
      "<var index> must be in [0, number of variables in file);\n"
      "-p <var1 index> <var2 index>,--onePair=<var1 index> <var2 index>     "
      "variable <var1 index> vs variable <var2 index> <var1 index> and <var2 "
      "index> must be in [0, number of variables in file);\n");
...
int MINE::TwoSetsAnalysis(double **inDataSet, int m, int n, int betweenid) {
  ...
  m_Results = new mine_result_score[outlen];
  int ret = mine_twoSetsAnalysis(&param, inDataSet, m, n, betweenid, m_Results,
                                 outlen);
  if (ret) {
    m_inputDataRowNum = m;
    m_inputDataColNum = n;
...
int MINE::MasterAnalysis(double **inData, int m, int n, int masterid) {
  ...
  int outlen = m;
  m_Results = new mine_result_score[outlen];
  int ret = mine_masterVariableAnalysis(&param, inData, m, n, masterid,
                                        m_Results, outlen);
  if (ret) {
    m_inputDataRowNum = m;
    m_inputDataColNum = n;
...
int MINE::run(int argc, char **argv) {
  if (!parserArgs(argc, argv))
    return 0;
  // read input data file;

  switch (m_inputLabel) {
  case 0:
    if (!readCSV(true, true))
      return 0;
    break;
  case 1:
    if (!readCSV(false, true))
      return 0;
    break;
  case 2:
    if (!readCSV(true, false))
      return 0;
    break;
  default:
    break;
  ...
  case AllParis:
    ...
    ret = AllPairsAnalysis(m_inputDataMatrix, m_inputDataRowNum,
                           m_inputDataColNum);
    break;
  case TwoSets:
    if (m_betweenId > 0 && m_betweenId < m_inputDataRowNum) {
      cout << "calculating..." << endl;
      ret = TwoSetsAnalysis(m_inputDataMatrix, m_inputDataRowNum,
                            m_inputDataColNum, m_betweenId);
    } else {
      cout << "input variable <var index> must be in (0, number of variables "
              "in file)"
           << endl;
      return 0;
    }
  ...
  case MasterVariable:
    if (m_masterId >= 0 && m_masterId < m_inputDataRowNum) {
      cout << "calculating..." << endl;
      ret = MasterAnalysis(m_inputDataMatrix, m_inputDataRowNum,
                           m_inputDataColNum, m_masterId);
    } else {
      cout << "input variable <var index> must be in [0, number of variables "
              "in file)"
           << endl;
    ...
    break;
  case OneParis:
    if (m_onepair1 < m_inputDataRowNum && m_onepair1 >= 0 && m_onepair2 >= 0 &&
        m_onepair2 < m_inputDataRowNum) {
      cout << "calculating..." << endl;
      ret = OnePairAnalysis(m_inputDataMatrix[m_onepair1],
                            m_inputDataMatrix[m_onepair2], m_inputDataColNum);
    } else {
      cout << "input variable <var index> must be in [0, number of variables "
              "in file)"
           << endl;
  ...
  if (!m_inputFileName.empty()) {
    ...
    while (getline(in, line) && in.good()) {
      ...
      for (int i = 0; i < strv.size(); i++) {
        if ((isFirstLine && collabel && rowlabel && i != 0) ||
            (isFirstLine && collabel && !rowlabel)) {
...
bool MINE::parserArgs(int argc, char **argv) {
  ...
  const Arg_parser::Option options[] = {{'V', "version", Arg_parser::no},
...
void MINE::printResult() {
  if (m_ResultsArrayLen > 0) {
    ...
    switch (m_AnalysisStyle) {
    case OneParis:
      std::cout << m_onepair1 << "," << m_onepair2 << "," << m_Results[0].mic
                << "," << m_Results[0].mev << "," << m_Results[0].mcn << ","
                << m_Results[0].mas << endl;
      break;
    case AllParis:
      for (int i = 0; i < m_inputDataRowNum; i++) {
        for (int j = i + 1; j < m_inputDataRowNum; j++) {
          int id = i * (m_inputDataRowNum - 1) - i * (i - 1) / 2 + j - i - 1;
          std::cout << i << "," << j << "," << m_Results[id].mic << ","
                    << m_Results[id].mev << "," << m_Results[id].mcn << ","
                    << m_Results[id].mas << endl;
        }
      }
      break;
    case MasterVariable:
      for (int i = 0; i < m_inputDataRowNum; i++) {
        if (i == m_masterId) {
          ...
          std::cout << m_masterId << "," << i << "," << m_Results[i].mic << ","
                    << m_Results[i].mev << "," << m_Results[i].mcn << ","
                    << m_Results[i].mas << endl;
        }
      }
      break;
    case TwoSets:
  ...
  if (m_ResultsArrayLen > 0) {
    if (m_varRowNames.size() > 0 && m_outputLabel == 1) {
      ...
      if (outputfile.is_open()) {
        ...
        switch (m_AnalysisStyle) {
        case OneParis:
          outputfile << m_varRowNames[m_onepair1] << ","
                     << m_varRowNames[m_onepair2] << "," << m_Results[0].mic
                     << "," << m_Results[0].mev << "," << m_Results[0].mcn
                     << "," << m_Results[0].mas << endl;
          break;
        case AllParis:
          for (int i = 0; i < m_inputDataRowNum; i++) {
            for (int j = i + 1; j < m_inputDataRowNum; j++) {
              int id =
                  i * (m_inputDataRowNum - 1) - i * (i - 1) / 2 + j - i - 1;
              outputfile << m_varRowNames[i] << "," << m_varRowNames[j] << ","
                         << m_Results[id].mic << "," << m_Results[id].mev << ","
                         << m_Results[id].mcn << "," << m_Results[id].mas
                         << endl;
            }
          }
          break;
        case MasterVariable:
          for (int i = 0; i < m_inputDataRowNum; i++) {
            if (i == m_masterId) {
              ...
              outputfile << m_varRowNames[m_masterId] << "," << m_varRowNames[i]
                         << "," << m_Results[i].mic << "," << m_Results[i].mev
                         << "," << m_Results[i].mcn << "," << m_Results[i].mas
                         << endl;
            }
          }
          break;
        ...
        cout << "Output file can not be created!";
        return false;
      }
      outputfile.close();
      ...
      if (outputfile.is_open()) {
        ...
        case OneParis:
#ifdef _MATLAB
          outputfile << (m_onepair1 + 1) << "," << (m_onepair2 + 1) << ","
                     << m_Results[0].mic << "," << m_Results[0].mev << ","
                     << m_Results[0].mcn << "," << m_Results[0].mas << endl;
#else
          outputfile << m_onepair1 << "," << m_onepair2 << ","
                     << m_Results[0].mic << "," << m_Results[0].mev << ","
                     << m_Results[0].mcn << "," << m_Results[0].mas << endl;
#endif
          break;
          ...
          for (int i = 0; i < m_inputDataRowNum; i++) {
            for (int j = i + 1; j < m_inputDataRowNum; j++) {
              int id =
                  i * (m_inputDataRowNum - 1) - i * (i - 1) / 2 + j - i - 1;
#ifdef _MATLAB
              outputfile << i + 1 << "," << j + 1 << "," << m_Results[id].mic
                         << "," << m_Results[id].mev << "," << m_Results[id].mcn
                         << "," << m_Results[id].mas << endl;
#else
              outputfile << i << "," << j << "," << m_Results[id].mic << ","
                         << m_Results[id].mev << "," << m_Results[id].mcn << ","
                         << m_Results[id].mas << endl;
#endif
            }
...
#ifdef _MATLAB
              outputfile << m_masterId + 1 << "," << i + 1 << ","
                         << m_Results[i].mic << "," << m_Results[i].mev << ","
                         << m_Results[i].mcn << "," << m_Results[i].mas << endl;
#else
              outputfile << m_masterId << "," << i << "," << m_Results[i].mic
                         << "," << m_Results[i].mev << "," << m_Results[i].mcn
                         << "," << m_Results[i].mas << endl;
