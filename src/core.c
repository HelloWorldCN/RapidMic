...
i = 0;
curr = 0;
rowsize =  (double) n / (double)
y;//每一格多少个数据点，在扫描过程中进行一定的动态调整，要保证相同值的数据落在同一格中
while (i < n)
{
s = 1;
//优化
...
{
curr++;
rowsize = (double) (n-i) / (double)
(y-curr);//因为上一格数据可能由于刚好截断的时候有相同值的数据，因此剩余的数据可能就要少了，所以需要重新计算剩余数据平均每格放多少个数据
}
  ...
  while (i < n) {
    ...
    if ((h != 0) && (fabs(h + s - rowsize) >= fabs(h - rowsize))) //一格数据满了
    {
      ...
      rowsize =
          (double)(n - i) /
          (double)(y -
                   curr); //因为上一格数据可能由于刚好截断的时候有相同值的数据，因此剩余的数据可能就要少了，所以需要重新计算剩余数据平均每格放多少个数据
    }
    for (j = 0; j < s; j++) {
      Qm[i + j] = curr;
    }
...
int GetClumpsPartition(double *Dx, int n, int *Qm) {
  ...
  while (
      i <
      n) //排序后数据序列中相同值的数中至少有一个如果没有划分到一个格子中，则相同的数重新划分到一个跟现有所有格子不同的新格子中
  {
 ...
 */
void sort(double *x, int *idx, int n) { quicksort(x, idx, 0, n - 1); }
